...
import { SponsorProfile, SponsorSponsorship } from "./types";
import { Avatar } from "@heroui/react";
import CreateSponsorshipDrawer from "../sponsorships/CreateSponsorshipDrawer";
import { formatSinceDate } from "../children/ChildSponsorsTab";
...
export default function SponsorSponsorshipsTab({ sponsor }: Props) {
	...
	async function fetchChildren(id: string) {
		...
		try {
			const res = await fetch(`/api/supabase/sponsorships/sponsor/${id}`);
	...
	useEffect(() => {
		setLoading(true);
		fetchChildren(sponsor.id).finally(() => setLoading(false));
	}, [sponsor.id]);
	...
	return (
		<div className="space-y-6">
								...
								<div className="flex items-start justify-between">
									...
									<div>
										...
										<p className="text-sm text-gray-400">
											...
											{formatSinceDate(s.start_date_time)}
										</p>
									</div>
								</div>
			...
			<CreateSponsorshipDrawer
				...
				onSaved={() => fetchChildren(sponsor.id)}
				sponsor={{
					id: sponsor.id,
